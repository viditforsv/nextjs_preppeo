-- Seed questions from lid_013 into question_bank and create quiz linkages.
-- Topic: Algebra / Linear Growth & Decay Models
-- Depends on: 20250302100012_sat_quant_learn_lid013_content.sql
...
BEGIN
  ...
  WHERE c.slug = 'sat-quant-learn'
    AND l.slug = 'sat-quant-learn-sat-lid-013'
  LIMIT 1;

  IF v_lesson_id IS NULL THEN
    RAISE NOTICE 'Lesson sat-quant-learn-sat-lid-013 not found; skipping.';
    RETURN;
  END IF;
  ...
  IF v_quiz_arr IS NULL OR jsonb_array_length(v_quiz_arr) = 0 THEN
    RAISE NOTICE 'No quiz data found for lid-013; skipping.';
    RETURN;
  END IF;
  ...
  FOR v_idx IN 0 .. jsonb_array_length(v_quiz_arr) - 1 LOOP
    ...
    INSERT INTO public.question_bank (
      ...
      v_diff, 'mcq', 'SAT Math', 'SAT', 'Algebra', 'Linear Growth & Decay Models',
      false, true, '12'
    )
  ...
  IF v_quiz_id IS NULL THEN
    ...
    VALUES ('lid_013 Practice', v_lesson_id)
  ...
  RAISE NOTICE 'Done lid_013: % questions seeded into question_bank', jsonb_array_length(v_quiz_arr);
END $$;
