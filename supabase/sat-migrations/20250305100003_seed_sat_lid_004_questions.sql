-- Seed questions from lid_004 into question_bank and create quiz linkages.
-- Topic: Algebra / Slope-Intercept and Standard Form
-- Depends on: 20250302100003_sat_quant_learn_lid004_content.sql
...
BEGIN
  ...
  WHERE c.slug = 'sat-quant-learn'
    AND l.slug = 'sat-quant-learn-sat-lid-004'
  LIMIT 1;

  IF v_lesson_id IS NULL THEN
    RAISE NOTICE 'Lesson sat-quant-learn-sat-lid-004 not found; skipping.';
    RETURN;
  END IF;
  ...
  IF v_quiz_arr IS NULL OR jsonb_array_length(v_quiz_arr) = 0 THEN
    RAISE NOTICE 'No quiz data found for lid-004; skipping.';
    RETURN;
  END IF;
  ...
  FOR v_idx IN 0 .. jsonb_array_length(v_quiz_arr) - 1 LOOP
    ...
    INSERT INTO public.question_bank (
      ...
      v_diff, 'mcq', 'SAT Math', 'SAT', 'Algebra', 'Slope-Intercept and Standard Form',
      false, true, '12'
    )
  ...
  IF v_quiz_id IS NULL THEN
    ...
    VALUES ('lid_004 Practice', v_lesson_id)
  ...
  RAISE NOTICE 'Done lid_004: % questions seeded into question_bank', jsonb_array_length(v_quiz_arr);
END $$;
