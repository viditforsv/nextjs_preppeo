/**
 * TeX bank → cbse10_maths_questions practice seed.
 *
 * Usage:
 *   node scripts/generate-cbse10m-lid-practice-sql.mjs --lid=1
 *   node scripts/generate-cbse10m-lid-practice-sql.mjs --lid=1 --out=path.sql
 *   node scripts/generate-cbse10m-lid-practice-sql.mjs --generate-backfill
 *   node scripts/generate-cbse10m-lid-practice-sql.mjs --lid=1 --tex-dir=path/to/tex
 */
...
const UNIT_TO_DOMAIN = {
  ...
  'pair of linear equations in two variables': 'linear-equations',
  ...
  'some applications of trigonometry': 'applications-of-trig',
  'applications of trigonometry': 'applications-of-trig',
  'circles': 'circles',
...
function parseArgs() {
  ...
  if (!lid) {
    console.error('Usage: node scripts/generate-cbse10m-lid-practice-sql.mjs --lid=NNN [--out=path] [--domain=override] [--tex-dir=path]');
    console.error('       node scripts/generate-cbse10m-lid-practice-sql.mjs --generate-backfill [--tex-dir=path]');
    process.exit(1);
  }
  ...
  if (!out) {
    ...
    out = path.join(root, 'supabase', 'migrations', 'cbse10', `${ts}_seed_cbse10m_lid_${slug}.sql`);
  }
...
function parseQuestion(block, slug) {
  ...
  return {
    ...
    bank_item_id: `cbse10m_lid_${slug}_${n}`,
  };
}

const defaultTexDir = path.join(root, 'Docs for me', 'cbse10-maths', 'question_bank');
...
function resolveTexPath(slug, texDir) {
  ...
  const texCbse = path.join(texDir, `cbse10m_lid_${slug}.tex`);
...
function generateBackfill(texDir) {
  let sql = `-- Backfill domain, chapter, subtopic for all CBSE10 maths LID practice questions\n\n`;
  const texFiles = fs.readdirSync(texDir).filter((f) => f.endsWith('.tex')).sort();
  let count = 0;
  for (const file of texFiles) {
    const m = file.match(/(?:cbse10m_)?lid_(\d+)\.tex$/);
    if (!m) continue;
    ...
    sql += `UPDATE public.cbse10_maths_questions SET domain = '${meta.domain}', chapter = ${ch}, subtopic = ${st}\n`;
    sql += `  WHERE bank_item_id LIKE 'cbse10m_lid_${slug}_%';\n\n`;
    count++;
  }
  const outPath = path.join(root, 'supabase', 'migrations', 'cbse10', '20260321200000_backfill_cbse10m_domain_chapter_subtopic.sql');
...
if (args.generateBackfill) {
  ...
  const meta = extractHeaderMeta(raw);
  const domain = domainOverride ?? meta.domain ?? 'real-numbers';
  ...
  const header = `-- CBSE 10 Maths practice from lid_${slug}.tex (cbse10m_lid_${slug})
-- domain=${domain}, chapter=${chapter ?? 'N/A'}, subtopic=${subtopic ?? 'N/A'}

INSERT INTO public.cbse10_maths_questions (
