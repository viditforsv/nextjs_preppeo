...
import type {
  IBDPMathsQuestion,
  IBDPMathsPracticeConfig,
  IBDPPracticePhase,
  IBDPMathsCourseConfig,
} from '@/types/ibdp-maths';

export interface IBDPMathsState {
  phase: IBDPPracticePhase;
  practiceConfig: IBDPMathsPracticeConfig | null;
  practiceQuestions: IBDPMathsQuestion[];
  ...
  goToAnalytics: () => void;
  startPracticeMode: (config: IBDPMathsPracticeConfig) => Promise<void>;
...
export function createIBDPMathsStore(courseConfig: IBDPMathsCourseConfig) {
  return create<IBDPMathsState>((set, get) => ({
    phase: 'landing',
    practiceConfig: null,
    practiceQuestions: [],
    practiceAnswers: {},
    ...
    startPracticeMode: async (config) => {
      ...
      const res = await fetch(`/api/${courseConfig.apiPrefix}/practice-questions?${params}`);
      if (!res.ok) {
    ...
    revealAnswer: (qId) => {
      ...
      fetch(`/api/${courseConfig.apiPrefix}/record-answer`, {
        method: 'POST',
