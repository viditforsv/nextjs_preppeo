...
export default function PracticeSummary() {
  ...
  const handlePracticeAgain = () => {
    useCBSE10MathsStore.setState({ phase: 'practice-config' });
  };
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={goToLanding}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm bg-white/10 hover:bg-white/20 rounded-lg transition-colors"
            >
              <ArrowLeft className="w-3.5 h-3.5" /> Back to CBSE
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-4">
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="text-center mb-6">
            <p className="text-sm text-gray-500 mb-1">CBSE 10 Maths Practice</p>
            <p className="text-5xl font-bold" style={{ color: '#8a8c00' }}>{pct}%</p>
                ...
                return (
                  <div key={domain}>
                    ...
                    <div className="w-full bg-gray-100 rounded-full h-2">
                      <div className="h-2 rounded-full transition-all" style={{ width: `${domPct}%`, backgroundColor: '#d4d700' }} />
                    </div>
                  </div>
                );
              })}
