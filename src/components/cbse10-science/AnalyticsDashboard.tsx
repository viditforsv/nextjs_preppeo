...
import type { AnalyticsData } from '@/types/cbse10-science';
...
export default function AnalyticsDashboard() {
  const { goToLanding } = useCBSE10ScienceStore();
  const [data, setData] = useState<AnalyticsData | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('/api/cbse10-science/analytics')
      .then((r) => r.json())
      .then(setData)
      .catch(() => setData(null))
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-4">
        {loading && (
          <div className="flex items-center justify-center py-16">
            <Loader2 className="w-6 h-6 animate-spin" style={{ color: ACCENT_DARK }} />
          </div>
        )}
          ...
          <>
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <div className="text-center mb-4">
                ...
                <p className="text-5xl font-bold" style={{ color: ACCENT_DARK }}>{overallAccuracy}%</p>
