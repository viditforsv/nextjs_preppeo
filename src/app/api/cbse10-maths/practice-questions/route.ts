...
import type { CBSE10MathsQuestion, QuestionType, CBSE10MathsDomain, DifficultyTier, QuestionOption } from '@/types/cbse10-maths';
...
function toQuestion(row: Record<string, unknown>): CBSE10MathsQuestion {
  return {
    id: row.id as string,
    type: row.type as QuestionType,
    difficulty: row.difficulty_tier as DifficultyTier,
    domain: (row.domain as CBSE10MathsDomain) ?? undefined,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const questions: CBSE10MathsQuestion[] = [];
    ...
    const { data: claimResult } = await supabase.rpc('claim_practice_usage', {
      p_user_id: user.id, p_date: today, p_easy: served.easy, p_medium: served.medium, p_hard: served.hard, p_course: 'cbse10-maths',
    });
    ...
    console.error('Error in GET /api/cbse10-maths/practice-questions:', error);
