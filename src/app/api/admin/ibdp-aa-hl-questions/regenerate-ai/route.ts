...
const TABLE = 'ibdp_aa_hl_questions';
const COURSE_LABEL = 'IB Mathematics Analysis & Approaches HL';
...
Correct answer: ${q.correct_answer}`;
    ...
    if (mode === 'theory') {
      systemPrompt = `You are an expert ${COURSE_LABEL} tutor. Provide the underlying theory behind this question.

${questionContext}

Provide a focused theory explanation (2-3 paragraphs):
1. Name and explain the specific concept, theorem, or formula this question tests.
2. Explain the mathematical concept with the general formula or method, showing how it applies broadly.
3. Give a clear, reusable strategy for tackling similar questions in IB exams.

Use plain text. For math, use Unicode symbols (², √, ×, ÷, π) instead of LaTeX. Be thorough but concise.`;
      maxTokens = 800;
    } else {
      systemPrompt = `You are an expert ${COURSE_LABEL} tutor. Explain this question step by step.
...
Use plain text. For math, use Unicode symbols (², √, ×, ÷, π) instead of LaTeX. Keep it conversational and encouraging.`;
      maxTokens = 500;
    }
