...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: q, error: fetchErr } = await supabase
      .from('gre_questions')
      .select('id, type, prompt, options, correct_answer, explanation, quantity_a, quantity_b, quantity_info, difficulty_tier, topics')
      .eq('id', questionId)
      .single();
    ...
    const qcContext = q.type === 'quantitative-comparison'
      ? `Quantity A: ${q.quantity_a ?? 'N/A'}\nQuantity B: ${q.quantity_b ?? 'N/A'}${q.quantity_info ? `\nAdditional info: ${q.quantity_info}` : ''}`
      : '';

    const questionContext = `Question type: ${typeLabel[q.type] ?? q.type}
Difficulty: ${q.difficulty_tier ?? 'Unknown'}
Topics: ${(q.topics ?? []).join(', ') || 'Unknown'}
${qcContext ? `\n${qcContext}\n` : ''}
Question: ${q.prompt}
${optionsText ? `\nOptions:\n${optionsText}` : ''}
Correct answer: ${q.correct_answer}`;
    ...
    if (mode === 'theory') {
      systemPrompt = `You are an expert GRE Quantitative Reasoning tutor. Provide the underlying theory behind a GRE Quant question.
...
2. Explain the mathematical concept with the general formula or method, showing how it applies broadly.
...
Use plain text. For math, use Unicode symbols (², √, ×, ÷) instead of LaTeX. Be thorough but concise.`;
      ...
      systemPrompt = `You are an expert GRE Quantitative Reasoning tutor. Explain a GRE Quant question.
...
2. Show the key step(s) to solve it.
...
Use plain text. For math, use Unicode symbols (², √, ×, ÷) instead of LaTeX. Keep it conversational and encouraging.`;
    ...
    await supabase.from('gre_questions').update({ [cacheCol]: text }).eq('id', questionId);

    return NextResponse.json({ text, success: true });
  } catch (err) {
    console.error('Error in POST /api/admin/gre-questions/regenerate-ai:', err);
