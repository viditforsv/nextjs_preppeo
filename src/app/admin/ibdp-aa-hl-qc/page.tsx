...
type IBDPQCQuestion = QCQuestion & {
  domain?: string;
  chapter?: string;
  subtopic?: string;
};
...
const config: QCPageConfig<IBDPQCQuestion> = {
  title: 'IB AA HL Questions QC',
  breadcrumbLabel: 'IB AA HL QC',
  subtitle: 'Review and edit all IB Mathematics Analysis & Approaches HL questions.',
  apiBase: '/api/admin/ibdp-aa-hl-questions',
  hasImageUpload: true,
  ...
  filters: [
    {
      key: 'domain',
      label: 'Topic',
      options: [{ value: 'all', label: 'All' }, ...IBDP_DOMAIN_OPTIONS],
    },
    {
      key: 'difficulty',
      label: 'Difficulty',
  ...
  sortFn: (a, b) => {
    const domOrder = (d: string | undefined) => Object.keys(IBDP_DOMAIN_LABELS).indexOf(d ?? '') ?? 99;
  ...
  metadataBadges: [
    {
      ...
      render: (q) => q.domain ? (
        <Badge className="bg-blue-100 text-blue-800 text-xs">
          {IBDP_DOMAIN_LABELS[q.domain as keyof typeof IBDP_DOMAIN_LABELS] ?? q.domain}
        </Badge>
      ) : null,
    },
    {
      key: 'difficulty',
      render: (q) => (
        <Badge className={DIFFICULTY_COLORS[q.difficulty] ?? 'bg-gray-100 text-gray-700'}>
    ...
    {
      key: 'type',
      render: (q) => <Badge variant="outline" className="text-xs">{q.type.toUpperCase()}</Badge>,
    },
    {
      key: 'chapter',
      render: (q) => q.chapter ? (
        <span className="text-xs text-gray-500">{q.chapter}{q.subtopic ? ` › ${q.subtopic}` : ''}</span>
      ) : null,
    },
    {
      key: 'qcDone',
        ...
        <div className="mt-3 space-y-1">
          {q.options.map((opt) => (
            <div key={opt.id} className={`text-sm px-3 py-1.5 rounded ${opt.id === q.correctAnswer ? 'bg-green-50 text-green-700 font-medium' : 'text-gray-600'}`}>
              <strong>{opt.id})</strong> {renderMixedContent(opt.text)}
...
export default function IBDPAAHLQCPage() {
