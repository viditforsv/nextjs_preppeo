...
export default function CBSE10ScienceQCPage() {
  const [questions, setQuestions] = useState<ScienceQCQuestion[]>([]);
  ...
  const [draft, setDraft] = useState<Partial<ScienceQCQuestion>>({});
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch('/api/admin/cbse10-science-questions');
  ...
  const merged = useMemo(() => {
    ...
    return { ...current, ...draft } as ScienceQCQuestion;
  }, [current, draft]);
  ...
  const handleImageUpload = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const res = await fetch('/api/admin/cbse10-science-questions/upload-image', { method: 'POST', body: form });
        if (!res.ok) {
          const data = await res.json();
  ...
  const removeImage = useCallback(async () => {
    ...
    setUploading(true);
    try {
      const res = await fetch('/api/admin/cbse10-science-questions/upload-image', {
        method: 'DELETE',
  ...
  const handleRegenerate = useCallback(
    async (mode: 'answer' | 'theory') => {
      ...
      try {
        const res = await fetch('/api/admin/cbse10-science-questions/regenerate-ai', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const updateDraft = (key: keyof ScienceQCQuestion, value: unknown) => {
      ...
      <div className="min-h-screen bg-background pb-20">
        <div className="container mx-auto px-4 py-8">
          <div className="mb-6">
            <Breadcrumb
              items={[
                ...
                { label: 'CBSE 10 Science QC', isActive: true },
              ]}
            />
          </div>
          ...
          <div className="mb-6 flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-foreground">CBSE 10 Science QC</h1>
              <p className="text-muted-foreground mt-1">
                Review and edit CBSE Class 10 Science questions.{' '}
                {editMode ? (
            ...
            <div className="max-w-3xl mx-auto space-y-4">
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input type="text" value={draft.chapter ?? current.chapter ?? ''} onChange={(e) => updateDraft('chapter', e.target.value)} className="w-full border border-gray-300 rounded-md px-2 py-1 text-sm" />
                    </div>
                    <div>
                      <label className="text-xs font-medium text-gray-600">Subtopic</label>
                      <input type="text" value={draft.subtopic ?? current.subtopic ?? ''} onChange={(e) => updateDraft('subtopic', e.target.value)} className="w-full border border-gray-300 rounded-md px-2 py-1 text-sm" />
                    </div>
                  </div>
                  <div>
                    <label className="text-xs font-medium text-gray-600">Correct Answer</label>
                    <input type="text" value={draft.correctAnswer ?? current.correctAnswer} onChange={(e) => updateDraft('correctAnswer', e.target.value)} className="w-full border border-gray-300 rounded-md px-2 py-1 text-sm font-mono" />
                  </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
                {editMode ? (
                  <div>
                    ...
                    <textarea value={draft.prompt ?? current.prompt} onChange={(e) => updateDraft('prompt', e.target.value)} rows={4} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm mt-1 font-mono" />
                  </div>
                ) : (
                  merged.prompt && (
                  ...
                  merged.options && merged.options.length > 0 && (
                    <div className="space-y-2">
                      {merged.options.map((opt) => {
                        ...
                        return (
                          <div key={opt.id} className={`flex items-start gap-3 p-3 rounded-lg border-2 ${isCorrect ? 'border-green-400 bg-green-50' : 'border-gray-200'}`}>
                            <span className={`w-7 h-7 rounded-full border-2 flex items-center justify-center text-xs font-bold shrink-0 mt-0.5 ${isCorrect ? 'border-green-500 bg-green-500 text-white' : 'border-gray-300 text-gray-500'}`}>{opt.id}</span>
                ...
                {editMode ? (
                  <div>
                    ...
                    <textarea value={draft.explanation ?? current.explanation ?? ''} onChange={(e) => updateDraft('explanation', e.target.value)} rows={4} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm mt-1 font-mono" />
                  </div>
                ) : (
                  merged.explanation && (
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button onClick={() => handleRegenerate('answer')} disabled={regenLoading !== null} className="inline-flex items-center gap-1 text-xs font-medium text-blue-600 hover:text-blue-800 disabled:opacity-50">
                      {regenLoading === 'answer' ? <Loader2 className="w-3 h-3 animate-spin" /> : <RotateCcw className="w-3 h-3" />}
                      Regenerate
                    </button>
                  </div>
                  {editMode ? (
                    <textarea value={draft.aiExplanation ?? current.aiExplanation ?? ''} onChange={(e) => updateDraft('aiExplanation', e.target.value)} rows={5} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm font-mono" placeholder="No AI explanation cached." />
                  ) : (
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button onClick={() => handleRegenerate('theory')} disabled={regenLoading !== null} className="inline-flex items-center gap-1 text-xs font-medium text-indigo-600 hover:text-indigo-800 disabled:opacity-50">
                      {regenLoading === 'theory' ? <Loader2 className="w-3 h-3 animate-spin" /> : <RotateCcw className="w-3 h-3" />}
                      Regenerate
                    </button>
                  </div>
                  {editMode ? (
                    <textarea value={draft.aiTheory ?? current.aiTheory ?? ''} onChange={(e) => updateDraft('aiTheory', e.target.value)} rows={5} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm font-mono" placeholder="No AI theory cached." />
                  ) : (
              ...
              {editMode && (
                <div className="flex items-center gap-3 bg-yellow-50 border border-yellow-200 rounded-xl p-4">
                  ...
                  <button onClick={handleDiscard} className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-lg border border-gray-300 bg-white hover:bg-gray-50 transition-colors">
                    Discard
                  </button>
              ...
              <div className="flex items-center justify-between">
                <button onClick={goPrev} disabled={currentIndex === 0} className="inline-flex items-center gap-1.5 px-5 py-2.5 text-sm font-medium rounded-lg border border-gray-300 bg-white hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
                  <ChevronLeft className="w-4 h-4" /> Previous
                </button>
                <span className="text-sm font-semibold text-gray-700 tabular-nums">{currentIndex + 1} / {filtered.length}</span>
                <button onClick={goNext} disabled={currentIndex === filtered.length - 1} className="inline-flex items-center gap-1.5 px-5 py-2.5 text-sm font-medium rounded-lg border border-gray-300 bg-white hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
        ...
        {!loading && !error && filtered.length > 0 && (
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-50">
            <div className="max-w-3xl mx-auto flex items-center justify-between px-4 py-3">
              <button onClick={goPrev} disabled={currentIndex === 0} className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-lg border border-gray-300 bg-white hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
                <ChevronLeft className="w-4 h-4" /> Previous
              </button>
              ...
              <button onClick={goNext} disabled={currentIndex === filtered.length - 1} className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-lg border border-gray-300 bg-white hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
                Next <ChevronRight className="w-4 h-4" />
              </button>
            </div>
          </div>
