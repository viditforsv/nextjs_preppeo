...
type MathsQCQuestion = QCQuestion & {
  domain?: string;
  chapter?: string;
  subtopic?: string;
  bankItemId?: string;
...
const config: QCPageConfig<MathsQCQuestion> = {
  title: 'CBSE 10 Maths QC',
  breadcrumbLabel: 'CBSE 10 Maths QC',
  subtitle: 'Review and edit CBSE Class 10 Mathematics questions.',
  apiBase: '/api/admin/cbse10-maths-questions',
  hasImageUpload: true,
    ...
    },
    {
      ...
      deriveOptions: (questions) => {
        const chapters = Array.from(
          new Set(questions.map((q) => (q as MathsQCQuestion).chapter).filter(Boolean) as string[])
        ).sort();
    ...
    },
    {
      ...
      deriveOptions: (questions) => {
        const subtopics = Array.from(
          new Set(questions.map((q) => (q as MathsQCQuestion).subtopic).filter(Boolean) as string[])
        ).sort();
  ...
  editFields: [
    {
      key: 'domain' as keyof MathsQCQuestion & string,
    ...
    {
      key: 'chapter' as keyof MathsQCQuestion & string,
      label: 'Topic (chapter)',
      type: 'text',
    },
    {
      key: 'subtopic' as keyof MathsQCQuestion & string,
      label: 'Subtopic',
      type: 'text',
    },
    {
    ...
    {
      key: 'options' as keyof MathsQCQuestion & string,
      label: 'Options (JSON)',
      type: 'json',
      rows: 6,
      colSpan: 3,
