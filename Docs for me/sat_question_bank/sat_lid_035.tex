...
% Question Begin
% =============================================================
% Lid 034: Problem-Solving & Data Analysis
% Topic: One-Variable Data: Standard Deviation & Box Plots
...
\begin{tabular}{|c|c|}
\hline
Number of employees & Number of restaurants \\ \hline
2 to 7 & 2 \\ \hline
8 to 13 & 4 \\ \hline
14 to 19 & 2 \\ \hline
20 to 25 & 7 \\ \hline
...
The median of a data set with $n$ values is the middle value. For $n=17$ restaurants, the median is the $9^{th}$ value ($ \frac{17+1}{2} = 9 $). We find which interval contains the $9^{th}$ value by calculating cumulative frequencies.
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[ybar interval, ymin=0, ymax=12, xmin=10, xmax=65, ylabel={Frequency}, xlabel={Integer}, xtick={10,20,30,40,50,60}, width=0.45\textwidth, title={Data Set A}]
\addplot coordinates {(10,0) (20,3) (30,4) (40,7) (50,9) (60,0)};
\end{axis}
\end{tikzpicture}
\begin{tikzpicture}
\begin{axis}[ybar interval, ymin=0, ymax=12, xmin=10, xmax=65, ylabel={Frequency}, xlabel={Integer}, xtick={10,20,30,40,50,60}, width=0.45\textwidth, title={Data Set B}]
\addplot coordinates {(10,3) (20,4) (30,7) (40,9) (50,0) (60,0)};
\end{axis}
\end{tikzpicture}
\end{center}
Two data sets of 23 integers each are summarized in the histograms shown. What is the smallest possible difference between the mean of data set A and the mean of data set B?
\begin{tasks}(4)
    \task 0
    \task 1
    \task 10
...
2. \textbf{Median:} The total number of values is $a+2a+3a+2a+a = 9a$. The median is the $(\frac{9a+1}{2})^{th}$ value, which falls in the "3" category. \\
3. \textbf{Mean:} Calculate the weighted average:
\[ \text{Mean} = \frac{1(a) + 2(2a) + 3(3a) + 4(2a) + 5(a)}{9a} = \frac{a + 4a + 9a + 8a + 5a}{9a} = \frac{27a}{9a} = 3 \]
...
% Category: Exam Level | Difficulty: Medium | Question Type: MCQ
\question
Two different teams consisting of 10 members each ran in a race.
Team A Mean: 3.41 minutes \\
Team B Mean: 3.79 minutes \\
Which of the following MUST be true?
\begin{tasks}(1)
    \task Every member of team A completed the race in less time than any member of team B.
    \task The median time of team B is greater than the median time of team A.
    \task There is at least one member of team B who took more time than some member of team A.
\end{tasks}
\begin{tasks}(4)
    \task III only
    \task I and III only
...
\begin{solution}
\textbf{Calculation:} \\
1. \textbf{Total Sum (8 players):} $8 \times 14.5 = 116$. \\
2. \textbf{Total Sum (7 players):} $7 \times 12 = 84$. \\
...
Answer: \colorbox{yellow!100}{3630}
\end{solution}
...
\foreach \x/\y in {1/4, 2/5, 3/2, 4/1, 5/1} {
    \foreach \i in {1,...,\y} \fill (\x, \i*0.2) circle (2pt);
}
\node at (3,-0.8) {Number of bursts};
\end{tikzpicture}
\end{center}
The dot plot above represents a data set of 13 eruptions. If an additional eruption with 11 bursts is added to this data set, which of the following measures will be greater for the new data set? \\
...
\hline
Station 1 & Station 2 & Station 3 & Station 4 & Station 5 \\ \hline
\$3.699 & \$3.609 & \$3.729 & \$3.679 & \$3.729 \\ \hline
\end{tabular}
\end{center}
Melissa recorded gas prices from five stations. What is the median of the gas prices?
\begin{tasks}(4)
    \task \$3.679
    \task \$3.689
    \task \$3.699
...
\begin{center}
\begin{tikzpicture}
\node at (-1,0.5) {Class A};
\draw[help lines] (1,0) -- (7,0);
\foreach \x in {1,...,7} \draw (\x,0.1) -- (\x,-0.1) node[below] {\x};
\foreach \x in {1,2} \fill (\x, 0.2) circle (2pt);
\foreach \x in {3} \foreach \i in {1,2,3} \fill (\x, \i*0.2) circle (2pt);
\foreach \x in {4} \foreach \i in {1,2,3,4} \fill (\x, \i*0.2) circle (2pt);
\foreach \x in {5} \foreach \i in {1,2,3,4,5} \fill (\x, \i*0.2) circle (2pt);
\foreach \x in {6} \foreach \i in {1,2,3,4,5,6} \fill (\x, \i*0.2) circle (2pt);
\foreach \x in {7} \foreach \i in {1,2,3,4,5,6,7} \fill (\x, \i*0.2) circle (2pt);

\begin{scope}[shift={(0,-2)}]
\node at (-1,0.5) {Class B};
\draw[help lines] (14,0) -- (20,0);
\foreach \x in {14,...,20} \draw (\x,0.1) -- (\x,-0.1) node[below] {\x};
\foreach \x in {14,15} \fill (\x, 0.2) circle (2pt);
\foreach \x in {16} \foreach \i in {1,2,3} \fill (\x, \i*0.2) circle (2pt);
\foreach \x in {17} \foreach \i in {1,2,3,4} \fill (\x, \i*0.2) circle (2pt);
\foreach \x in {18} \foreach \i in {1,2,3,4,5} \fill (\x, \i*0.2) circle (2pt);
\foreach \x in {19} \foreach \i in {1,2,3,4,5,6} \fill (\x, \i*0.2) circle (2pt);
\foreach \x in {20} \foreach \i in {1,2,3,4,5,6,7} \fill (\x, \i*0.2) circle (2pt);
\end{scope}
\end{tikzpicture}
\end{center}
Which statement best compares the standard deviations of the number of glue sticks brought in for these two classes?
\begin{tasks}(1)
    \task SD of Class A is less than Class B.
    \task SD of Class A is equal to Class B.
    \task SD of Class A is greater than Class B.
    \task Not enough information.
\end{tasks}
...
\question
Data set A: 10, 11, 11, 11, 11, 12, 12, 13, 13, 13, 14, 14, 15, 15, 15, 15, 16 \\
Data set B: 10, 10, 11, 11, 11, 11, 12, 12, 13, 14, 14, 15, 15, 15, 15, 16, 16 \\
Which statement must be true?
\begin{tasks}(1)
    \task The median of A is equal to the median of B.
    \task The standard deviation of A is equal to the standard deviation of B.
\end{tasks}
\begin{tasks}(4)
    \task I only
    \task II only
...
\node[above] at (5,1.5) {Group X};
\draw (1,0.5) -- (2,0.5); \draw (7,0.5) -- (9,0.5); % Whiskers
\draw (2,0.2) rectangle (7,0.8); \draw (4,0.2) -- (4,0.8); % Box
\begin{scope}[shift={(0,-1.5)}]
\node[above] at (5,1.5) {Group Y};
\draw (2,0.5) -- (3,0.5); \draw (6,0.5) -- (8,0.5); % Whiskers
\draw (3,0.2) rectangle (6,0.8); \draw (4,0.2) -- (4,0.8); % Box
\end{scope}
\end{tikzpicture}
\end{center}
The box plots above summarize the scores of two groups. Which of the following must be true?
\begin{tasks}(1)
    \task Group X has a larger range and a larger IQR than Group Y.
    \task Group X has a smaller range and a larger IQR than Group Y.
    \task Group X has a larger range and a smaller IQR than Group Y.
    \task The means of Group X and Group Y are equal.
\end{tasks}
...
\textbf{Calculation and Logic:} \\
1. \textbf{New Mean:} $40 \times 3 = 120$. \\
2. \textbf{New SD:} $5 \times 3 = 15$.
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[ybar interval, ymin=0, ymax=10, xmin=0, xmax=50, ylabel={Frequency}, xlabel={Value}, xtick={0,10,20,30,40,50}, width=0.6\textwidth]
\addplot coordinates {(0,2) (10,8) (20,5) (30,3) (40,2) (50,0)};
\end{axis}
\end{tikzpicture}
\end{center}
For the data summarized in the histogram above, which of the following is true?
\begin{tasks}(1)
    \task The mean is likely greater than the median.
    \task The mean is likely less than the median.
    \task The mean and median are exactly equal.
...
\draw (1,0.5) -- (2,0.5); \draw (6,0.5) -- (7,0.5); 
\draw (2,0.2) rectangle (6,0.8); \draw (5,0.2) -- (5,0.8);
\end{tikzpicture}
\end{center}
Based on the box plot above, what is the interquartile range (IQR)?
\begin{tasks}(4)
    \task 4
    \task 5
    \task 6
...
\begin{solution}
\textbf{Calculation:} \\
Sum = Mean $\times n = 25 \times 4 = 100$. 
$10 + 20 + 30 + x = 100$. 
$60 + x = 100 \Rightarrow x = 40$.
...
\draw (0,0) -- (10,0);
\foreach \x in {0,2,4,6,8,10} \draw (\x,0.1) -- (\x,-0.1) node[below] {\x};
\node[left] at (0,1) {Set A};
\draw (1,1) -- (2,1); \draw (8,1) -- (9,1);
\draw (2,0.7) rectangle (8,1.3); \draw (5,0.7) -- (5,1.3);

\begin{scope}[shift={(0,-1.5)}]
\node[left] at (0,1) {Set B};
\draw (4,1) -- (4.5,1); \draw (5.5,1) -- (6,1);
\draw (4.5,0.7) rectangle (5.5,1.3); \draw (5,0.7) -- (5,1.3);
\end{scope}
\end{tikzpicture}
\end{center}
...
\begin{tasks}(1)
    \task It increases by 10.
    \task It decreases by 10.
    \task It stays the same.
    \task It is multiplied by 10.
\end{tasks}
...
\begin{tikzpicture}
\draw (0,0) -- (6,0);
\foreach \x in {0,1,2,3,4,5,6} \draw (\x,0.1) -- (\x,-0.1) node[below] {\x};
\foreach \x/\y in {1/1, 2/2, 3/5, 4/2, 5/1} {
    \foreach \i in {1,...,\y} \fill (\x, \i*0.2) circle (2pt);
}
\end{tikzpicture}
\end{center}
For the dot plot above, which of the following is true?
\begin{tasks}(1)
    \task Mean = Median
    \task Mean > Median
    \task Mean < Median
    \task Standard Deviation = 0
\end{tasks}
...
\begin{tasks}(1)
    \task The mean stays the same, and the standard deviation stays the same.
    \task The mean stays the same, and the standard deviation decreases.
    \task The mean increases, and the standard deviation stays the same.
    \task The mean stays the same, and the standard deviation increases.
\end{tasks}
...
\textbf{Calculation and Logic:} \\
1. \textbf{Subtraction (Shifting):} Subtracting 50 from every value moves the entire data set 50 units to the left on a number line. However, the \textit{distance between the points} remains exactly the same. Therefore, subtracting 50 has \textbf{no effect} on the standard deviation.
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[ybar interval, ymin=0, ymax=10, xmin=0, xmax=40, ylabel={Frequency}, xlabel={Age}, xtick={0,10,20,30,40}]
\addplot coordinates {(0,9) (10,5) (20,2) (30,1) (40,0)};
\end{axis}
\end{tikzpicture}
\end{center}
Which of the following is the most likely relationship between the mean and the median for the data set shown in the histogram?
\begin{tasks}(1)
    \task mean $\approx$ median
    \task mean $<$ median
    \task mean $>$ median
    \task standard deviation = 0
\end{tasks}
...
\[ \text{New IQR} = \text{Original IQR} \times 3 \]
3. \textbf{Calculate:} 
\[ 12 \times 3 = 36 \]
...
% Category: Exam Level | Difficulty: Hard | Question Type: MCQ
\question
Data set P: \{2, 4, 6, 8, 10\} \\
Data set Q: \{2, 2, 6, 10, 10\} \\
Which of the following is true?
\begin{tasks}(1)
    \task Set P has a larger standard deviation than Set Q.
    \task Set Q has a larger standard deviation than Set P.
    \task The means of the two sets are different.
    \task The medians of the two sets are different.
\end{tasks}
...
\draw (0,0) -- (6,0);
\foreach \x in {1,2,3,4,5} \draw (\x,0.1) -- (\x,-0.1) node[below] {\x};
\foreach \x/\y in {1/2, 2/2, 3/2, 4/2, 5/2} {
    \foreach \i in {1,...,\y} \fill (\x, \i*0.2) circle (2pt);
}
\end{tikzpicture}
\end{center}
In the uniform dot plot shown above, how does the mean compare to the median?
\begin{tasks}(4)
    \task Mean $>$ Median
    \task Mean $<$ Median
    \task Mean = Median
...
3. \textbf{Find Mean:} 
\[ \frac{2(1) + 2(2) + 2(3) + 2(4) + 2(5)}{10} = \frac{2 + 4 + 6 + 8 + 10}{10} = \frac{30}{10} = 3 \]
...
\[ \text{Sum} = \text{Mean} \times n = 12 \times 5 = 60 \]
...
\[ \frac{1+2+9+10+10}{5} = \frac{32}{5} = 6.4 \]
...
\textbf{Calculation and Logic:} \\
1. \textbf{Multiply Value $\times$ Frequency:}
   - $5 \times 8 = 40$
   - $10 \times 4 = 40$
   - $15 \times 8 = 120$
...
\begin{tasks}(2)
    \task Group A
    \task Group B
    \task Both are equally consistent.
    \task Cannot be determined.
\end{tasks}
