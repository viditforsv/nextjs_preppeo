...
\begin{center}
    ...
    By Preppeo & \normalsize\bf{{sat\_lid\_043}} & SAT\\
    \normalsize{{\textbf{{Unit:}} Lines \& Angles}} & \normalsize{{\textbf{{Chapter:}} Lines, angles, parallel/perpendicular lines}} & \normalsize{{\textbf{{Topic:}} Angles in Polygons}} \\
    \end{tabularx}
\end{center}
...
\begin{center}
\begin{tikzpicture}
\draw[thick] (0,0) -- (3,0) -- (4,2) -- (2,3.5) -- (-1,2) -- cycle;
\node at (-0.2,0.3) {$x^\circ$};
\node at (3.2,0.3) {$110^\circ$};
\node at (3.8,1.8) {$120^\circ$};
\node at (2,3.1) {$100^\circ$};
\node at (-0.8,1.8) {$115^\circ$};
\end{tikzpicture}
\end{center}
What is the value of $x$ in the pentagon shown above?
\begin{tasks}(4)
    \task 95
    \task 105
    \task 115
...
\begin{solution}
\textbf{Conceptual Explanation:} \\
A regular polygon has sides of equal length and interior angles of equal measure. The formula for the measure of one interior angle of a regular $n$-gon is $\frac{(n - 2) \times 180}{n}$.

\vspace{30pt}

\textbf{Calculation and Logic:} \\
First, identify the number of sides for a hexagon, which is $n = 6$. Second, calculate the total sum of the interior angles using $(6 - 2) \times 180$. This becomes $4 \times 180 = 720^\circ$. Third, since the hexagon is regular, all 6 interior angles are equal. Divide the total sum by the number of angles: $720 / 6$. Performing the division results in 120. Therefore, each interior angle of a regular hexagon measures 120 degrees.
...
\begin{center}
\begin{tikzpicture}
\draw[thick] (0,0) -- (4,0) -- (3,2.5) -- (1,2.5) -- cycle;
\node at (0.3,0.3) {$x^\circ$};
\node at (3.7,0.3) {$x^\circ$};
\node at (1.2,2.2) {$y^\circ$};
\node at (2.8,2.2) {$y^\circ$};
\end{tikzpicture}
\end{center}
In the isosceles trapezoid shown above, $y = 2x + 30$. What is the value of $x$?
\begin{tasks}(4)
    \task 50
    \task 60
    \task 70
...
\begin{solution}
\textbf{Conceptual Explanation:} \\
We use the interior angle sum formula, $S = (n - 2) \times 180$, and solve for the variable $n$, which represents the number of sides.

\vspace{30pt}

\textbf{Calculation and Logic:} \\
Set up the equation using the given sum: $1440 = (n - 2) \times 180$. First, divide both sides of the equation by 180 to simplify: $1440 / 180 = n - 2$. Calculating the division gives $8 = n - 2$. Next, isolate $n$ by adding 2 to both sides of the equation: $n = 8 + 2 = 10$. This means the polygon is a decagon with 10 sides.
...
\begin{solution}
\textbf{Conceptual Explanation:} \\
For a regular polygon, all exterior angles are equal. Since the total sum of exterior angles is always $360^\circ$, the measure of one exterior angle is $\frac{360}{n}$.

\vspace{30pt}

\textbf{Calculation and Logic:} \\
Set up the relationship: $40 = \frac{360}{n}$. To solve for $n$, multiply both sides by $n$ to get $40n = 360$. Then, divide both sides by 40: $n = 360 / 40$. Performing the division gives $n = 9$. The polygon is a regular nonagon with 9 sides.
...
\begin{center}
\begin{tikzpicture}
\draw[thick] (0,0) -- (2,0) -- (3,1.73) -- (2,3.46) -- (0,3.46) -- (-1,1.73) -- cycle;
\draw[thick] (0,0) -- (3,1.73);
\node at (0.4,0.2) {$a^\circ$};
\end{tikzpicture}
\end{center}
In the regular hexagon shown above, a diagonal is drawn from one vertex to another. What is the value of $a$?
\begin{tasks}(4)
    \task 30
    \task 60
    \task 90
...
\begin{solution}
\textbf{Conceptual Explanation:} \\
The sum of the interior angles of any quadrilateral is $(4 - 2) \times 180 = 360^\circ$. The missing angle is found by subtracting the sum of the known angles from 360.
...
\begin{solution}
\textbf{Conceptual Explanation:} \\
The relationship between the number of sides $n$ and the sum of the interior angles $S$ is defined by the formula $S = (n - 2) \times 180$. To find the number of sides, we set the given sum equal to this formula and solve for $n$.

\vspace{30pt}

\textbf{Calculation and Logic:} \\
First, set up the equation: $1080 = (n - 2) \times 180$. To isolate the term $(n - 2)$, divide both sides of the equation by 180. The calculation $1080 / 180$ results in 6. This gives the simplified equation $6 = n - 2$. Finally, add 2 to both sides to solve for $n$. The calculation $6 + 2$ results in 8. Thus, the polygon has 8 sides, making it an octagon.
...
\begin{center}
\begin{tikzpicture}
\draw[thick] (0,0) -- (2,0) -- (2.5,1.5) -- (1,2.5) -- (-0.5,1.5) -- cycle;
\node at (1,0.3) {$(3x)^\circ$};
\node at (1.8,1) {$(4x)^\circ$};
\node at (1,2) {$(2x)^\circ$};
\node at (0.2,1) {$(4x)^\circ$};
\node at (1,-0.3) {Pentagon $P$};
\end{tikzpicture}
\end{center}
If the five interior angles of pentagon $P$ are $3x, 4x, 2x, 4x,$ and $5x$, what is the value of $x$?
\begin{tasks}(4)
    \task 18
    \task 30
    \task 36
...
\begin{solution}
\textbf{Conceptual Explanation:} \\
The sum of the interior angles of a pentagon is always $(5-2) \times 180 = 540^\circ$. To find the value of a variable within these angles, sum the algebraic expressions and set them equal to 540.
...
\textbf{Calculation and Logic:} \\
Step 1: Find the number of sides $n$. $(n - 2) \times 180 = 1800$. Dividing by 180 gives $n - 2 = 10$, so $n = 12$. Step 2: Divide the total sum by the number of angles. $\text{One Angle} = 1800 / 12$. Performing the division results in 150. Each interior angle of the regular 12-gon measures 150 degrees.
...
\begin{tasks}(1)
    \task It increases.
    \task It decreases.
    \task It stays the same ($360^\circ$).
    \task It becomes greater than the interior angle.
    \end{tasks}
...
\begin{solution}
\textbf{Conceptual Explanation:} \\
The number of diagonals $D$ in a polygon with $n$ sides is given by the formula $D = \frac{n(n - 3)}{2}$. We solve for $n$ by setting $D = 20$.

\vspace{30pt}

\textbf{Calculation and Logic:} \\
Set up the equation: $20 = \frac{n(n - 3)}{2}$. Multiply both sides by 2: $40 = n^2 - 3n$. Rearrange into a quadratic equation: $n^2 - 3n - 40 = 0$. Factoring the quadratic: $(n - 8)(n + 5) = 0$. Since the number of sides must be positive, $n = 8$. The polygon is an octagon.
...
\draw[thick] (0,2) -- (1.5,3) -- (3,2);
\node at (1.5,1) {House Shape};
\end{tikzpicture}
\end{center}
A "house" shape is formed by placing an equilateral triangle on top of a square. What is the measure of the interior angle formed where the triangle meets the square?
\begin{tasks}(4)
    \task $90^\circ$
    \task $120^\circ$
    \task $150^\circ$
...
\begin{solution}
\textbf{Calculation and Logic:} \\
Use the formula $(n - 2) \times 180$. For a quadrilateral, $n = 4$. $(4 - 2) \times 180 = 2 \times 180 = 360^\circ$.
...
\begin{solution}
\textbf{Calculation and Logic:} \\
Total sum: $(15 - 2) \times 180 = 13 \times 180 = 2,340^\circ$. Measure of one angle: $2,340 / 15$. Performing the division results in 156. Each interior angle is 156 degrees.

\vspace{30pt}

Answer: \colorbox{yellow!100}{156}
\end{solution}
...
\begin{solution}
\textbf{Calculation and Logic:} \\
$(n - 2) \times 180 = 2700$. Divide by 180: $n - 2 = 15$. Add 2: $n = 17$.
...
\begin{solution}
\textbf{Calculation and Logic:} \\
Hexagon interior sum: $(6 - 2) \times 180 = 720^\circ$. Sum of five known angles: $100 + 110 + 120 + 130 + 140 = 600^\circ$. Sixth angle: $720 - 600 = 120^\circ$.
...
\begin{solution}
\textbf{Calculation and Logic:} \\
Exterior sum is always $360^\circ$. Interior sum of a quadrilateral is $(4 - 2) \times 180 = 360^\circ$. Therefore, the quadrilateral is the only polygon where these two sums are equal.
...
\begin{solution}
\textbf{Calculation and Logic:} \\
Exterior angle: $180 - 150 = 30^\circ$. Number of sides $n = 360 / 30 = 12$. Total interior sum: $(12 - 2) \times 180 = 10 \times 180 = 1,800^\circ$.
...
\begin{tasks}(1)
    \task Yes, it would be a triangle.
    \task Yes, it would be a heptagon.
    \task No, because the exterior angle would not divide 360 evenly.
    \task No, because $n$ would be less than 3.
\end{tasks}
...
\begin{solution}
\textbf{Calculation and Logic:} \\
Sum of angles: $x + 2x + 3x + 4x = 10x$. Set equal to 360: $10x = 360 \Rightarrow x = 36$. The smallest angle is $x = 36^\circ$.
...
\begin{solution}
\textbf{Calculation and Logic:} \\
$(20 - 2) \times 180 = 18 \times 180 = 3,240^\circ$.
...
\begin{solution}
\textbf{Calculation and Logic:} \\
$x + (2x + 60) = 180 \Rightarrow 3x = 120 \Rightarrow x = 40$. Exterior angle is 40. $n = 360 / 40 = 9$.
...
\begin{solution}
\textbf{Calculation and Logic:} \\
A heptagon has $n = 7$ sides. $(7 - 2) \times 180 = 5 \times 180 = 900^\circ$.
...
\begin{solution}
\textbf{Calculation and Logic:} \\
$7x + 2x = 180 \Rightarrow 9x = 180 \Rightarrow x = 20$. Exterior angle = $2(20) = 40^\circ$. Number of sides $n = 360 / 40 = 9$.
...
\begin{solution}
\textbf{Calculation and Logic:} \\
$D = \frac{n(n - 3)}{2}$. For $n = 8$: $D = \frac{8(5)}{2} = 40 / 2 = 20$.
...
\textbf{Calculation and Logic:} \\
Identify the number of sides, which is $n = 14$. Apply the formula: $S = (n - 2) \times 180$. Substitute 14 for $n$: $S = (14 - 2) \times 180$. Simplify the subtraction: $S = 12 \times 180$. Perform the multiplication: $12 \times 100 = 1200$ and $12 \times 80 = 960$. Adding these together, $1200 + 960 = 2,160$. The total sum is 2,160 degrees.

\vspace{30pt}

Answer: \colorbox{yellow!100}{2160}
\end{solution}
...
\begin{solution}
\textbf{Conceptual Explanation:} \\
The sum of interior angles must be a multiple of 180 degrees because the formula is $(n-2) \times 180$, where $n-2$ must be a positive integer.
...
\begin{solution}
\textbf{Conceptual Explanation:} \\
We use the diagonal formula $D = \frac{n(n-3)}{2}$ and the given ratio to set up an algebraic equation to solve for $n$.

\vspace{30pt}

\textbf{Calculation and Logic:} \\
The ratio of diagonals to sides is $D/n = 2/1$. Substitute the formula for $D$: $\frac{n(n-3)}{2} \div n = 2$. Simplify the left side by canceling out $n$: $\frac{n-3}{2} = 2$. Multiply both sides by 2: $n - 3 = 4$. Add 3 to both sides: $n = 7$. The polygon has 7 sides.
...
\textbf{Calculation and Logic:} \\
Step 1: Find the number of sides $n$. $n = 360 / 12 = 30$. Step 2: Find the interior sum using $n = 30$. $S = (30 - 2) \times 180$. $S = 28 \times 180$. Perform the multiplication: $28 \times 18 = 504$. Therefore, $28 \times 180 = 5,040$. The total interior sum is 5,040 degrees.

\vspace{30pt}

Answer: \colorbox{yellow!100}{5040}
\end{solution}
...
\textbf{Calculation and Logic:} \\
Step 1: Find the exterior angle. $180 - 150 = 30^\circ$. Step 2: Find the number of sides $n$. $n = 360 / 30 = 12$. Step 3: Calculate the perimeter. $\text{Perimeter} = n \times \text{side length}$. $\text{Perimeter} = 12 \times 5 = 60$. The perimeter of the regular 12-gon is 60 units.
