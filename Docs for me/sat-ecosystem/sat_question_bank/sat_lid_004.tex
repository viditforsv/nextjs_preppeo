...
\begin{tasks}(2)
    \task $12.50h \leq 100$
    \task $35 + 12.50h \leq 100$
    \task $35h + 12.50 \leq 100$
    \task $12.50 + 35h \geq 100$
\end{tasks}
...
\\[10pt]
The phrase "no more than" indicates that the total cost must be less than or equal to the tourist's budget of \$100. In mathematical notation, this is expressed using the $\leq$ symbol. Therefore, the complete inequality representing the constraint on the rental time is $35 + 12.50h \leq 100$.
...
\begin{tasks}(2)
    \task $12 \leq n \leq 15$
    \task $12 \leq n \leq 180$
    \task $15 \leq n \leq 180$
    \task $12 \leq n \leq 15$
\end{tasks}
...
\\[10pt]
Because the width can be any value between 12 and 15, the resulting number of docks $n$ must fall within the range of 12 to 15, inclusive. This relationship is expressed by the inequality $12 \leq n \leq 15$.
...
\begin{tasks}(2)
    \task $18s + 22w \leq 500$; $w \geq 3s$
    \task $18s + 22w \leq 500$; $s \geq 3w$
    \task $40(s+w) \leq 500$; $w \geq 3s$
    \task $18s + 22w \geq 500$; $w \leq 3s$
\end{tasks}

\begin{solution}
This problem requires two separate inequalities to model the truck's capacity and the company's purchasing preference. First, we determine the weight constraint. The total weight is the sum of the weight of the soda crates ($18s$) and the weight of the water crates ($22w$). Since the truck has a maximum capacity of 500 pounds, the total weight must be less than or equal to 500, giving us $18s + 22w \leq 500$.
\\[10pt]
Second, we model the relationship between the quantities of the two items. The company requires "at least three times as many water crates as soda crates." This means the number of water crates, $w$, must be greater than or equal to three times the number of soda crates, $s$. This is written mathematically as $w \geq 3s$. 
\\[10pt]
...
\\[10pt]
The total time spent driving is simply the sum of the hours driven by each person, which is $a + b$. The problem states that this total time was "under 6 hours." In mathematical terms, "under" means strictly less than, so we use the $<$ symbol rather than the $\leq$ symbol.
\\[10pt]
...
% Category: Practice | Difficulty: Medium | Question Type: Multiple Choice
\question
Which point $(x, y)$ is a solution to the system of inequalities $y \leq 2x + 4$ and $y \geq -x + 1$?
\begin{tasks}(4)
    \task $(0, 0)$
    \task $(0, 5)$
    \task $(2, 1)$
...
\\[10pt]
Let's test the coordinates $(2, 1)$. For the first inequality, $y \leq 2x + 4$, we substitute 2 for $x$ and 1 for $y$. This gives $1 \leq 2(2) + 4$, which simplifies to $1 \leq 8$. This is a true statement. Next, we test the same point in the second inequality, $y \geq -x + 1$. Substituting the values gives $1 \geq -(2) + 1$, which simplifies to $1 \geq -1$. This is also a true statement.
\\[10pt]
...
\begin{tasks}(2)
    \task $0.2x + 0.6y \geq 50$
    \task $20x + 60y \geq 50$
    \task $0.2x + 0.6y \leq 50$
    \task $x + y \geq 50$
\end{tasks}
...
\\[10pt]
The problem specifies that the bowl must contain "at least 50 grams" of peanuts. The phrase "at least" means the amount must be 50 grams or more, which we represent with the $\geq$ symbol. Combining these parts results in the inequality $0.2x + 0.6y \geq 50$.
...
\\[10pt]
The final inequality that shows the constraint on the number of hours the plumber can work within the given budget is $50 + 80h \leq 400$. 
...
\\[10pt]
If we had used the longest panels (8 feet), we would have only needed 12 panels ($96 / 8 = 12$). Since the question asks for the \textit{maximum} number of panels, we conclude that 16 is the correct answer.
...
\\[10pt]
If every box contains the minimum of 15 books, the total number of books would be $7 \times 15$, which equals 105. This represents the lower limit of the possible range. If every box contains the maximum of 22 books, the total would be $7 \times 22$, which equals 154. This represents the upper limit.
\\[10pt]
...
\begin{tasks}(2)
    \task $5x + 12y \geq 10$
    \task $0.05x + 0.12y \leq 10$
    \task $0.05x + 0.12y \geq 10$
    \task $0.5x + 1.2y \geq 10$
\end{tasks}
...
\\[10pt]
The requirement is to have "at least 10 pounds" of nitrogen. "At least" means that the amount can be exactly 10 or anything greater than 10. We use the "greater than or equal to" symbol ($\geq$) to denote this. Thus, the inequality is $0.05x + 0.12y \geq 10$.
...
\\[10pt]
Since the driver's budget is limited to \$20, the total cost must be less than or equal to 20. This leads to the inequality $5 + 3(h - 1) \leq 20$. This structure ensures that the \$5 is only charged once and the \$3 rate only applies to time spent after the first hour.
...
\begin{tasks}(2)
    \task $a + b \geq 50$; $4a + 6b \leq 400$
    \task $a + b \leq 50$; $4a + 6b \leq 400$
    \task $a + b \geq 50$; $4a + 6b \geq 400$
    \task $4a + 6b \leq 50$; $a + b \geq 400$
\end{tasks}
...
\\[10pt]
The quantity constraint states that the owner must buy "at least 50 bulbs total." The total number of bulbs is the sum of Type A ($a$) and Type B ($b$). "At least" means 50 or more, so we write $a + b \geq 50$.
\\[10pt]
The cost constraint involves the price of each bulb. Type A bulbs contribute $4a$ to the total cost, and Type B bulbs contribute $6b$. The sum $4a + 6b$ represents the total expenditure. Since the owner "cannot spend more than \$400," the total must be less than or equal to 400. This is written as $4a + 6b \leq 400$. Combining these two gives the correct system.
...
\begin{tasks}(2)
    \task $150 + 0.25m \leq 250$
    \task $150 + 0.25(m - 200) \leq 250$
    \task $150 + 200(m - 0.25) \leq 250$
    \task $0.25(m + 200) \leq 250$
\end{tasks}
...
\\[10pt]
The customer wants to spend "at most" \$250. This means the total cost must be less than or equal to 250. This gives us the final inequality $150 + 0.25(m - 200) \leq 250$.
...
\begin{tasks}(2)
    \task $a + b \geq 40$; $0.3a + 0.7b \leq 20$
    \task $a + b \leq 40$; $0.3a + 0.7b \geq 20$
    \task $a + b \geq 40$; $30a + 70b \leq 20$
    \task $0.3a + 0.7b \geq 40$; $a + b \leq 20$
\end{tasks}

\begin{solution}
We have two constraints here: the total volume of the mixture and the specific amount of alcohol within that volume.
\\[10pt]
The volume constraint is straightforward. The total liters $a + b$ must be "at least 40," which means 40 or more. This gives us $a + b \geq 40$.
\\[10pt]
The alcohol constraint requires us to look at the concentrations. Liquid A contributes $0.30a$ liters of alcohol, and Liquid B contributes $0.70b$ liters of alcohol. The sum of these, $0.3a + 0.7b$, represents the total alcohol in the mixture. The problem states this total must be "no more than 20 liters," which translates to less than or equal to 20. Thus, $0.3a + 0.7b \leq 20$. 
...
\\[10pt]
The athlete wants to burn "at least 600" calories. "At least" indicates that 600 is the minimum acceptable amount, so the total should be greater than or equal to 600. This is written as $12r + 8c \geq 600$.
...
\begin{tasks}(2)
    \task $5n + 2p > 20$; $5n + 2p \leq 50$
    \task $5n + 2p < 20$; $5n + 2p \geq 50$
    \task $n + p > 20$; $n + p \leq 50$
    \task $5n + 2p \geq 20$; $5n + 2p < 50$
\end{tasks}
...
\\[10pt]
We are given a range for the total spending. First, the cost must be "more than \$20." This is a strict inequality, so we use $5n + 2p > 20$. Second, the cost must be "no more than \$50." This means it can be exactly 50 or anything less, so we use $5n + 2p \leq 50$.
\\[10pt]
...
\\[10pt]
Since the passenger can spend a maximum of \$24, we set the cost to be less than or equal to 24: $4 + 2.5m \leq 24$. To solve for $m$, we first subtract the flat fee of 4 from both sides, leaving $2.5m \leq 20$.
\\[10pt]
...
% Category: Practice | Difficulty: Hard | Question Type: Multiple Choice
\question
Which point lies in the solution set of $y \leq -x + 10$, $y \geq 2$, and $x \geq 5$?
\begin{tasks}(4)
    \task (4, 4)
    \task (6, 1)
    \task (6, 3)
...
\\[10pt]
First, check (6, 3): 
1. Is $x \geq 5$? Since 6 is greater than 5, this is true.
2. Is $y \geq 2$? Since 3 is greater than 2, this is true.
3. Is $y \leq -x + 10$? Substituting the values gives $3 \leq -6 + 10$, which simplifies to $3 \leq 4$. This is also true.
\\[10pt]
Because (6, 3) satisfies all three inequalities, it is the correct answer. In contrast, (4, 4) fails the $x \geq 5$ check, (6, 1) fails the $y \geq 2$ check, and (11, 2) fails the $y \leq -x + 10$ check because 2 is not less than or equal to $-1$.
...
\\[10pt]
Now, divide both sides by 0.10. Dividing by 0.10 is the same as multiplying by 10. $20 \times 10 = 200$. Therefore, if you send more than 200 text messages, the second plan becomes the cheaper option.
...
\begin{tasks}(2)
    \task $40f + 25b \geq 2,000$
    \task $40f + 25b \leq 2,000$
    \task $65(f + b) \geq 2,000$
    \task $25f + 40b \geq 2,000$
\end{tasks}
...
\\[10pt]
The theater's goal is to make "at least \$2,000." "At least" indicates that the revenue should be \$2,000 or any amount higher than that. This is represented by the "greater than or equal to" symbol ($\geq$).
\\[10pt]
Combining these expressions gives us the inequality $40f + 25b \geq 2,000$.
...
\\[10pt]
If we plug in $x = 4$, we get $y = (3/4) \times 4$, which simplifies to $y = 3$. Since this matches the y-coordinate of the point (4, 3), that point must lie on the line.
...
\begin{solution}
First, find $b$ by plugging in the point (1, 5): $5 = 2(1) + b \Rightarrow b = 3$. The equation is $y = 2x + 3$.
\\[10pt]
To find the x-intercept, set $y = 0$: $0 = 2x + 3 \Rightarrow -3 = 2x \Rightarrow x = -1.5$.
\vspace{15pt}
Answer: \colorbox{yellow!100}{A}
\end{solution}
